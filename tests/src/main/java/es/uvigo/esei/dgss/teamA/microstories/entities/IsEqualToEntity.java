...
public abstract class IsEqualToEntity<T> extends TypeSafeMatcher<T> {
	...
	protected void addTemplatedDescription(final String attribute, final Object expected) {
		this.describeTo = d -> d.appendText(String.format("%s entity with value '%s' for %s",
				this.expected.getClass().getSimpleName(), expected, attribute));
	}
	...
	protected <R> boolean checkAttribute(final String attribute, final Function<T, R> getter, final T actual,
	 ...
	 * @param actual         the actual entity being compared to the expected
	 *                       entity.
	 ...
	 */
	protected <R> boolean checkArrayAttribute(final String attribute, final Function<T, R[]> getter, final T actual,
		...
		if (expectedValue == null && actualValue == null) {
			...
			final Matcher<Iterable<? extends R>> matcher = matcherFactory.apply(expectedValue);
	 ...
	 *         the current description will be updated.
	 */
	protected <R> boolean checkIterableAttribute(final String attribute, final Function<T, Iterable<R>> getter,
			final T actual, final Function<Iterable<R>, Matcher<Iterable<? extends R>>> matcherFactory) {
	...
	/**
	 * Compares the expected and the actual value of an attribute. If the comparison
	 * fails, the description of the error will be updated.
	 *
	 * @param attribute the name of the attribute compared.
	 * @param getter    the getter function of the attribute.
	...
	/**
	 * Compares the expected and the actual value of an int array attribute. If the
	 * comparison fails, the description of the error will be updated.
	 *
	 * @param attribute the name of the attribute compared.
	 * @param getter    the getter function of the attribute.
	 ...
	 *         the current description will be updated.
	 */
	protected boolean checkIntArrayAttribute(final String attribute, final Function<T, int[]> getter, final T actual) {
	...
	protected static <T> Matcher<Iterable<? extends T>> containsEntityInAnyOrder(
		...
		final Collection<Matcher<? super T>> entitiesMatchers = stream(entities).map(converter).collect(toList());
	...
	protected static <T> Matcher<Iterable<? extends T>> containsEntityInAnyOrder(
		...
		final Collection<Matcher<? super T>> entitiesMatchers = StreamSupport.stream(entities.spliterator(), false)
				.map(converter).collect(toList());
	 ...
	 *
	 * @param converter A function to create a matcher for an entity.
	 * @param entities  The entities to be used as the expected values, in the order
	 *                  to be compared.
	...
	protected static <T> Matcher<Iterable<? extends T>> containsEntityInOrder(
			final Function<T, Matcher<? super T>> converter, final Iterable<T> entities) {
		final List<Matcher<? super T>> matchersList = StreamSupport.stream(entities.spliterator(), false).map(converter)
				.collect(toList());
